...
type Project = { id: string; title: string; category: string; budget_min: number; budget_max: number; status: string; client_id: string }
      ...
      <div style={{ flex: 1, maxWidth: '900px', width: '100%', margin: '0 auto', padding: '32px 32px 0', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        {project && (
          <div style={{ backgroundColor: '#131929', border: '1px solid #1e2d4a', borderRadius: '12px', padding: '20px 24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <a href="/app/lamaran" style={{ color: '#8892a4', fontSize: '13px', textDecoration: 'none' }}>← Kembali</a>
            </div>
          </div>
        )}
        ...
        <div style={{ backgroundColor: '#131929', border: '1px solid #1e2d4a', borderRadius: '12px', overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: '450px' }}>
          ...
          <div style={{ padding: '16px 20px', borderTop: '1px solid #1e2d4a', display: 'flex', gap: '8px' }}>
            ...
            />
            <button onClick={handleSend} disabled={sending || !input.trim()} style={{ padding: '10px 20px', backgroundColor: input.trim() ? '#4F6EF7' : '#1e2d4a', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 'bold', cursor: input.trim() ? 'pointer' : 'default', transition: 'background 0.2s' }}>
              {sending ? '...' : 'Kirim'}
            </button>
          </div>
        </div>
