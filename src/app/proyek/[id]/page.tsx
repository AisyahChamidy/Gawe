...
export default function ProyekDetailPublikPage() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: proj }, { data: { user: u } }] = await Promise.all([
      ...
      const [{ data: profile }, { count }] = await Promise.all([
        supabase.from('profiles').select('full_name').eq('id', proj.client_id).single(),
      ...
      if (u) {
        const { data: app } = await supabase.from('applications')
          .select('id').eq('project_id', projectId).eq('freelancer_id', u.id).single()
        setSudahDilamar(!!app)
      }
  ...
  async function handleLamar() {
    ...
    const { error } = await supabase.from('applications').insert({
      project_id: projectId, freelancer_id: user.id, cover_letter: '', status: 'pending',
    })
  ...
  if (notFound || !project) return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0A0E1A', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'white', gap: '16px' }}>
      ...
      <h1 style={{ fontSize: '24px' }}>Proyek tidak ditemukan</h1>
      <a href="/proyek" style={{ color: '#4F6EF7', textDecoration: 'none' }}>← Kembali ke daftar proyek</a>
    </div>
  )
        ...
        <div style={{ position: 'sticky', top: '84px' }}>
          <div style={{ backgroundColor: '#131929', border: '1px solid #1e2d4a', borderRadius: '16px', padding: '24px' }}>
            ...
            {sudahDilamar || applied ? (
              ...
              <button onClick={handleLamar} disabled={applying} style={{ width: '100%', padding: '14px', backgroundColor: applying ? '#2a3a6a' : '#4F6EF7', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '700', fontSize: '15px', cursor: applying ? 'not-allowed' : 'pointer' }}>
                {applying ? 'Mengirim...' : 'Lamar Proyek'}
              </button>
            ) : (
              <a href="/auth/masuk" style={{ display: 'block', width: '100%', padding: '14px', backgroundColor: '#4F6EF7', color: 'white', textDecoration: 'none', borderRadius: '10px', fontWeight: '700', fontSize: '15px', textAlign: 'center', boxSizing: 'border-box' }}>
                Masuk untuk Melamar
              </a>
            )}
