...
export default function KlienProyekPage() {
  ...
  async function fetchProjects() {
    ...
    const { data } = await supabase
      .from('projects')
      .select(`
        id, title, category, budget_min, budget_max, status,
        applications (
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#0A0E1A', fontFamily: 'sans-serif', color: 'white' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
          <a href="/klien/proyek" style={{ color: 'white', textDecoration: 'none', fontSize: '14px', fontWeight: 'bold' }}>Proyekku</a>
              ...
              <div key={project.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ color: '#8892a4', fontSize: '13px' }}>
                        {project.applications?.length || 0} pelamar
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ padding: '16px 24px' }}>
                  {!project.applications || project.applications.length === 0 ? (
                    <p style={{ color: '#8892a4', fontSize: '14px', textAlign: 'center', padding: '20px' }}>
                      Belum ada pelamar
                    </p>
                  ) : (
                              ...
                              <>
                                <button
                                  ...
                                  style={{
                                    ...
                                    fontWeight: 'bold',
                                    cursor: 'pointer',
                                    opacity: actionLoading === app.id ? 0.7 : 1,
                                  }}
                                >
                                  Terima
                                </button>
                                <button
                                  onClick={() => handleTolak(app.id)}
                                  disabled={actionLoading === app.id}
                                  style={{
                                    padding: '6px 14px',
